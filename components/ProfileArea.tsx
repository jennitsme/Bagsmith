          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex flex-col items-center gap-3">
              ...
              <label className="text-xs md:text-sm font-mono brutal-border px-3 py-2 cursor-pointer hover:bg-[var(--surface-hover)] transition-colors flex items-center gap-2">
                ...
                <input type="file" accept="image/*" className="hidden" disabled={!wallet || uploading} onChange={(e) => e.target.files?.[0] && uploadAvatar(e.target.files[0])} />
              </label>
            </div>
            ...
            <div className="flex-1 space-y-4">
              <div>
                ...
                <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} disabled={!wallet} className="mt-1 w-full bg-[var(--bg)] brutal-border px-3 py-2 text-sm font-mono focus:outline-none" />
              </div>
              <div>
                <label className="font-mono text-xs text-[var(--text-muted)]">Bio</label>
                <textarea value={bio} onChange={(e) => setBio(e.target.value)} disabled={!wallet} className="mt-1 w-full bg-[var(--bg)] brutal-border px-3 py-2 text-sm font-mono focus:outline-none min-h-[90px]" />
              </div>
              <button onClick={saveProfile} disabled={!canSave || saving} className="px-4 py-2 font-bold uppercase tracking-wider text-xs md:text-sm brutal-border bg-[var(--neon)] text-black hover:bg-white transition-colors disabled:opacity-50">
